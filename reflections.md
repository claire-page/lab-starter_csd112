...
subtype of ConsoleLogger it's being called on. 
This is an example of polymorphism in action. yay polymorphism. 
...
Logger.log must be abstract because that's how you create the "contract" where you tell the compiler that all implementations of Logger WILL have a log method.
It allows the implementation of log to be distinct from subclass to subclass.
...
The compile-time type of the MemoryLogger when declared as Logger logger = new MemoryLogger(); is Logger-> Logger does not have an export method. 
However, if you change the declaration back to MemoryLogger logger = new MemoryLogger(); the compiler recognizes that MemoryLogger is able to call 
the export() method as it inherits it through the Exportable interface it implements. 
